...
class ToolbarWidgetState extends State<ToolbarWidget> {
  ...
  List<bool> _fontSelected = List<bool>.filled(7, false);

  /// List that controls which [ToggleButtons] are selected for
  /// forecolor/backcolor
  List<bool> _backgroundColorSelected = List<bool>.filled(1, false);

  /// List that controls which [ToggleButtons] are selected for
  /// forecolor/backcolor
  List<bool> _foregroundColorSelected = List<bool>.filled(1, false);
  ...
  void initState() {
    ...
    for (var t in widget.htmlToolbarOptions.defaultToolbarButtons) {
      if (t is FontButtons) {
        _fontSelected = List<bool>.filled(t.getIcons().length, false);
      }
      if (t is ColorButtons) {
        _backgroundColorSelected = List<bool>.filled(t.getIcons().length, false);
      }
      if (t is FontSettingButtons && t.foregroundColor) {
        _foregroundColorSelected = List<bool>.filled(1, false);
      }
      if (t is ListButtons) {
  ...
  Widget _createClickableIcon(IconData icon, {required VoidCallback? action}) {
    return InkWell(
        onTap: action,
        ...
        child: Icon(icon, color: action == null ? Colors.grey : null, size: 28));
  }
      ...
      for (var t in widget.htmlToolbarOptions.defaultToolbarButtons) {
        if (t is FontButtons) {
          for (var i = 0; i < _fontSelected.length; i++) {
            if (t.getIcons()[i].icon == Icons.format_bold) {
              _fontSelected[i] = fontList[0] ?? false;
            }
            if (t.getIcons()[i].icon == Icons.format_italic) {
              _fontSelected[i] = fontList[1] ?? false;
            }
            if (t.getIcons()[i].icon == Icons.format_underline) {
              _fontSelected[i] = fontList[2] ?? false;
            }
            if (t.getIcons()[i].icon == Icons.format_strikethrough) {
              _fontSelected[i] = miscFontList[0] ?? false;
            }
            if (t.getIcons()[i].icon == Icons.superscript) {
              _fontSelected[i] = miscFontList[1] ?? false;
            }
            if (t.getIcons()[i].icon == Icons.subscript) {
              _fontSelected[i] = miscFontList[2] ?? false;
            }
          }
        }
        if (t is ListButtons) {
  ...
  List<Widget> _buildChildren() {
    ...
    for (var t in widget.htmlToolbarOptions.defaultToolbarButtons) {
      ...
      if (t is FontSettingButtons) {
        ...
        if (t.foregroundColor || t.fontSize) {
          toolbarChildren.add(Row(
            children: [
              if (t.fontSize) ...[
                Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 8),
                    child: _createClickableIcon(Icons.text_increase,
                        action: _fontSizeSelectedItem < 7
                            ? () {
                                setState(mounted, this.setState, () {
                                  _fontSizeSelectedItem += 1;
                                });

                                widget.controller.execCommand('fontSize', argument: _fontSizeSelectedItem.toString());
                              }
                            : null)),
                _createClickableIcon(
                  Icons.text_decrease,
                  action: _fontSizeSelectedItem > 1
                      ? () {
                          setState(mounted, this.setState, () {
                            _fontSizeSelectedItem -= 1;
                          });
              ...
              ToggleButtons(
                ...
                onPressed: (int index) async {
                  void updateStatus(Color? color) {
                    setState(mounted, this.setState, () {
                      _foregroundColorSelected[index] = !_foregroundColorSelected[index];
                  ...
                  if (_foregroundColorSelected[index]) {
                    if (t.getIcons()[index].icon == Icons.format_color_text) {
                      var proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.foregroundColor, _foregroundColorSelected[index], updateStatus) ?? true;
                      if (proceed) {
                    ...
                    var proceed = true;
                    if (t.getIcons()[index].icon == Icons.format_color_text) {
                      proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.foregroundColor, _foregroundColorSelected[index], updateStatus) ?? true;
                    }
                    if (proceed) {
                      late Color newColor;
                      if (t.getIcons()[index].icon == Icons.format_color_text) {
                ...
                isSelected: _foregroundColorSelected,
                children: t.getIcons(),
              ),
            ],
          ));
      ...
      if (t is FontButtons) {
        if (t.bold || t.italic || t.underline || t.clearAll || t.strikethrough || t.superscript || t.subscript) {
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              ...
              if (t.getIcons()[index].icon == Icons.format_bold) {
              ...
              if (t.getIcons()[index].icon == Icons.format_italic) {
              ...
              if (t.getIcons()[index].icon == Icons.format_underline) {
              ...
              if (t.getIcons()[index].icon == Icons.format_clear) {
              ...
              if (t.getIcons()[index].icon == Icons.format_strikethrough) {
                var proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.strikethrough, _fontSelected[index], updateStatus) ?? true;
                if (proceed) {
                  widget.controller.execCommand('strikeThrough');
                  updateStatus();
                }
              }
              if (t.getIcons()[index].icon == Icons.superscript) {
                var proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.superscript, _fontSelected[index], updateStatus) ?? true;
                if (proceed) {
                  widget.controller.execCommand('superscript');
                  updateStatus();
                }
              }
              if (t.getIcons()[index].icon == Icons.subscript) {
                var proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.subscript, _fontSelected[index], updateStatus) ?? true;
                if (proceed) {
      ...
      if (t is ColorButtons && t.highlightColor) {
        toolbarChildren.add(ToggleButtons(
          constraints: BoxConstraints.tightFor(
            width: widget.htmlToolbarOptions.toolbarItemHeight - 2,
            height: widget.htmlToolbarOptions.toolbarItemHeight - 2,
          ...
          onPressed: (int index) async {
            void updateStatus(Color? color) {
              setState(mounted, this.setState, () {
                _backgroundColorSelected[index] = !_backgroundColorSelected[index];
            ...
            if (_backgroundColorSelected[index]) {
              if (t.getIcons()[index].icon == Icons.format_color_fill) {
                var proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.highlightColor, _backgroundColorSelected[index], updateStatus) ?? true;
                if (proceed) {
              ...
              var proceed = true;
              if (t.getIcons()[index].icon == Icons.format_color_fill) {
                proceed = await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.highlightColor, _backgroundColorSelected[index], updateStatus) ?? true;
              }
              if (proceed) {
                late Color newColor;
                if (t.getIcons()[index].icon == Icons.format_color_text) {
          ...
          isSelected: _backgroundColorSelected,
          children: t.getIcons(),
        ));
      }
      if (t is ListButtons) {
                              ...
                              TextButton(
                                onPressed: () async {
                                  if (formKey.currentState!.validate()) {
                                    var proceed = await widget.htmlToolbarOptions.linkInsertInterceptor?.call(text.text.isEmpty ? url.text : text.text, url.text, openNewTab) ?? true;
                                    if (proceed) {
      ...
      if (t is OtherFontSettingsButtons && t.fontName) {
        toolbarChildren.add(Container(
          padding: const EdgeInsets.only(left: 8.0),
          height: widget.htmlToolbarOptions.toolbarItemHeight,
          decoration: !widget.htmlToolbarOptions.renderBorder
